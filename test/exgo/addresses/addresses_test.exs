defmodule ExGo.AddressesTest do
  ...
  describe "links" do
    ...
    test "update_link/2 with valid data updates the link" do
      link = link_fixture()
      assert {:ok, %Link{} = link} = Addresses.update_link(link, @update_attrs)
    ...
    test "update_link/2 with invalid data returns error changeset" do
      link = link_fixture()
      assert {:error, %Ecto.Changeset{}} = Addresses.update_link(link, @invalid_attrs)
      assert link == Addresses.get_link!(link.id)
    end

    test "delete_link/1 deletes the link" do
      link = link_fixture()
      assert {:ok, %Link{}} = Addresses.delete_link(link)
      assert_raise Ecto.NoResultsError, fn -> Addresses.get_link!(link.id) end
    end
